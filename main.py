...
if useDB:
    ...
    prompt = Make_prompt.onePrompt(db_results, positive_type_list, negative_type_list)
...
filteredTS_list, filteredTypes_list = testscenariostoList(llm_TS.content)
...
types_list = copy.deepcopy(filteredTypes_list)
invalid_TS_list = []
...
print("\n-- Types -- ")
for index in range(len(filteredTypes_list)):
    print(f"{index + 1}. {filteredTypes_list[index]}")
print("\n--------------------------------\n")
...
print(f"Length of valid TS: {len(valid_TS_list)}")
print(f"Length of types: {len(types_list)}")
print(f"Length of invalid: {len(invalid_TS_list)}")
